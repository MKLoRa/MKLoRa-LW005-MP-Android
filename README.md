# LW005 Android SDK Guide（English）
...
**`com.moko.support.lw005.LoRaLW005MokoSupport`**
...
**`com.moko.support.lw005.OrderTaskAssembler`**

We assemble read data and write data to `OrderTask`, send the task to the device through `LoRaLW005MokoSupport `, and receive the resopnse.
...
First of all, you should initialize the LoRaLW005MokoSupport.We recommend putting it in Application.
...
Before operating the Bluetooth scanning device, we need to apply for permission, which we have added in LoRaLW005MokoSupport `AndroidManifest.xml`
...
```
LoRaLW005MokoSupport.getInstance().connDevice(beaconXInfo.mac);
```
...
```
LoRaLW005MokoSupport.getInstance().sendOrder(OrderTaskAssembler.setPassword(password));
...
```
// read lora region
LoRaLW005MokoSupport.getInstance().sendOrder(derTaskAssembler.getLoraRegion());
...
// get result
@Subscribe(threadMode = ThreadMode.MAIN)
public void onOrderTaskResponseEvent(OrderTaskResponseEvent event) {
...
2.`EventBus` is used in the SDK and can be modified in `LoRaLW005MokoSupport` if you want to use other communication methods.
